...
Spotify. It has three states — **connected**, **connecting**, and **error** — and surfaces the
signed-in account (name, plan, active device) plus a reconnect affordance when needed.
...
- **Connected:** a card with the account avatar/initials, name, a success check, and
  "`{Plan} · {Device}`" (e.g. "Premium · DESKTOP-AX31"), plus a green "Connected" label.
...
- [ ] Connected state shows account name, plan, and active device name.
...
- A `StatusViewModel` exposes a `ConnectionState` enum and an `Account` model (name, plan,
  device, initials) sourced from `GET /v1/me` and `GET /v1/me/player`
  (confirm fields via the OpenAPI spec, §6).
- Use `InfoBar` (`Severity = Informational/Error`) for connecting/error and a native card layout
  (`Border`/`Grid` with theme brushes) for connected. Use `FontIcon` glyphs; spinner =
  `ProgressRing`.
